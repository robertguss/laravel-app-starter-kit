...
| Community starter distribution implied a package release | The installer-supported Git URL path is certified; private access uses Tiged's documented Git mode; no Packagist package or custom generator was published | Installer `--using` behavior and release certification       |
